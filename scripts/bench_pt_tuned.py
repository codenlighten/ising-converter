"""Focused benchmark at N=200 and N=300: does auto-tuning the PT beta ladder
let it dominate SA cleanly in the hard-regime crossover band?

Compares four samplers on the same SK instances:
    ising_lab.SA            -- our reference SA
    ising_lab.PT            -- our PT with geometric ladder (default)
    ising_lab.PT-tuned      -- our PT with auto_tune_beta_ladder per instance
    neal.SA                 -- D-Wave's neal reference SA

Pilot cost for PT-tuned is included in its wall-time accounting.
"""
...
CSV_PATH = Path("results/pt_narrow_vs_wide.csv")
JSON_PATH = Path("results/pt_narrow_vs_wide.json")


def build_samplers():
    return {
        "ising_lab.SA": wrap_sa(num_sweeps=NUM_SWEEPS, seed=0),
        "ising_lab.PT-wide": wrap_pt(
            num_sweeps=NUM_SWEEPS, num_replicas=PT_REPLICAS,
            beta_min=0.1, beta_max=10.0, seed=0,
        ),
        "ising_lab.PT-narrow": wrap_pt(
            num_sweeps=NUM_SWEEPS, num_replicas=PT_REPLICAS,
            beta_min=0.3, beta_max=3.0, seed=0,   # informed endpoints, no tuning
        ),
...
def main():
    ...
    summarize(records, f"Hard regime (panel-best as truth, {NUM_SWEEPS} sweeps)")
    ...
    # Which sampler now holds the floor for each instance?
